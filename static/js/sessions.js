...
function createSessionItem(s) {
  ...
  // Extract Memories
  const _memIcon = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4.03 3 9 3s9-1.34 9-3V5"/><path d="M3 12c0 1.66 4.03 3 9 3s9-1.34 9-3"/></svg>';
  const extractItem = document.createElement('div');
  extractItem.className = 'dropdown-item-compact';
  extractItem.innerHTML = _icon(_memIcon) + '<span>Extract Memories</span>';
  extractItem.addEventListener('click', (e) => {
    e.stopPropagation();
    dropdown.style.display = 'none';
    document.dispatchEvent(new CustomEvent('extract-session-memories', { detail: { sessionId: s.id } }));
  });
  dropdown.appendChild(extractItem);
